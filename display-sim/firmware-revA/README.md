# Tide Display - STM32 Hardware (Rev A)

E-paper tide display for Margate, UK using STM32F411CE and 2.9" SSD1680 tri-color display.

**This is the Rev A firmware (tested and working).** See [firmware-revB](../firmware-revB/) for Rev B hardware.
...
- 15-minute update cycle with deep sleep (~uA standby current)

## Operation

The device wakes every 15 minutes (at :00, :15, :30, :45):
1. DS3231 alarm triggers wake via PA0 interrupt
2. LEDs turn on (indicates awake)
3. Read time from RTC
4. Render and update e-paper display
5. Set next alarm
6. LEDs turn off (indicates sleep)
7. Enter STOP mode

**LED behavior:**
- LEDs ON = awake and updating display
- LEDs OFF = in deep sleep mode
...
Time is set using a button via on-screen interface:
- Hold button on boot to enter time-setting mode
- Use button presses to adjust hours/minutes
- Time stored as local Margate time (GMT/BST)
...
firmware/
├── src/
│   ├── main.c              # Entry point, RTC, sleep/wake cycle
