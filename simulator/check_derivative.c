...
typedef struct { float amplitude; float omega; float phase_rad; } TideConstituent;
...
int main() {
    ...
    tm.tm_mday = 6;
    tm.tm_hour = 11;
    time64_t base = timegm(&tm);
    
    printf("Feb 6 2025, heights and derivatives around noon UTC:\n");
    printf("Time     Height  Deriv   (PLA LW at 11:52)\n");
    for (int m = 0; m <= 90; m += 5) {
        time64_t t = base + m * 60;
        struct tm *lt = gmtime((time_t*)&t);
        printf("%02d:%02d    %.3f   %+.4f\n", lt->tm_hour, lt->tm_min, calc_height(t), calc_deriv(t));
    }
    return 0;
}
